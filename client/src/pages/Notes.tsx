/* Petal Postcard design reminder: this route creates a playful handwritten-notes moment for Isbah,
   ...
   Personal photos and memories remain exclusively preserved for the kite reveal. */
...
const notes = [
  {
    title: "Your warmth",
    body: "You make ordinary moments feel soft, safe, and a little more golden.",
    icon: Heart,
    color: "blush",
  },
  {
    title: "Your magic",
    body: "You carry a kind of sparkle that cannot be copied. Please never make it smaller.",
    icon: Sparkles,
    color: "butter",
  },
  {
    title: "Your next chapter",
    body: "May it bring gentle surprises, loud laughter, and wishes that find you.",
    icon: Star,
    color: "rose",
  },
];
      ...
      <section className="screen notes-screen" aria-labelledby="notes-title">
              ...
              >
                ...
                <AnimatePresence mode="wait">
                  {isOpen ? (
                    ...
                    <motion.span className="note-cover" key="cover" exit={{ opacity: 0, y: -5 }}>
        ...
        <div className="page-actions two-actions">
          ...
          <button
            ...
            onClick={() => setLocation("/kite")}
            disabled={!allOpened}
          >
            <span>{allOpened ? "Let the kite fly" : `${opened.length}/3 notes opened`}</span>{" "}
            <ArrowRight size={16} />
          </button>
        </div>
      </section>
